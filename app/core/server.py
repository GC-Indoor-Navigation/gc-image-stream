...
STREAM_CAMERA_CREDENTIAL_STATUS_URL_TEMPLATE = get_optional_str_env(
    "STREAM_CAMERA_CREDENTIAL_STATUS_URL_TEMPLATE"
)
if STREAM_CAMERA_INGEST_AUTH_ENABLED and (
    "{credential_id}" not in STREAM_CAMERA_CREDENTIAL_STATUS_URL_TEMPLATE
):
    raise RuntimeError(
        "STREAM_CAMERA_CREDENTIAL_STATUS_URL_TEMPLATE must contain {credential_id} "
        "when camera ingest authentication is enabled"
    )
