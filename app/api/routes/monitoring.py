  ...
  <script>
    ...
    function renderIngest(ingest) {
      ...
      ingestGrid.innerHTML = [
        ...
        relayCard("Gate Start TS", ingest.gate_start_timestamp_ms ?? "-"),
        relayCard("First Accepted TS", ingest.first_accepted_timestamp_ms ?? "-"),
        relayCard("Pre-gate Drops", ingest.pre_gate_dropped_count ?? 0, Number(ingest.pre_gate_dropped_count || 0) > 0 ? "stale" : "healthy"),
        relayCard("Stale Gate Drops", ingest.stale_after_gate_dropped_count ?? 0, Number(ingest.stale_after_gate_dropped_count || 0) > 0 ? "stale" : "healthy"),
